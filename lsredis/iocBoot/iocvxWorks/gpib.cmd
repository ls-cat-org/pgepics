...
dbLoadRecords("$(IP)/ipApp/Db/Keithley2kDMM_mf.db","P=lsredis:,Dmm=D2,PORT=gpib1 2")

# send impromptu message to gpib device, parse reply
# (was GPIB_OI_block)
#dbLoadRecords("$(IP)/ipApp/Db/deviceCmdReply.db","P=lsredis:,N=1,PORT=gpib1,ADDR=1,OMAX=100,IMAX=100")
...
#dbLoadRecords("$(IP)/ipApp/Db/HeidAWE1024.db", "P=lsredis:,L=10,A=6")

# Keithley 199 DMM at GPIB address $(A)
#dbLoadRecords("$(STD)/stdApp/Db/KeithleyDMM.db", "P=lsredis:,L=10,A=26")

 ...
 */
static long value_init_stringin_record( stringinRecord *prec) {
  ...
  rvs = callocMustSucceed( 1, sizeof( *rvs), "redis value STRINGIN init");
  ...
  if( strlen( rvs->epicsPVBase) >= sizeof(prec->name)+1) {
    fprintf( stderr, "value_init_stringin_record: PV name is too short.  Name='%s', epicsPVBase='%s'\n", prec->name, rvs->epicsPVBase);
    return 1;
  }
  ...
  fprintf( stderr, "redis value stringin intit  pv '%s'  rediskey '%s'\n", prec->name, rvs->redisKey);
...
epicsExportAddress(dset,devStringinRedisValue);
